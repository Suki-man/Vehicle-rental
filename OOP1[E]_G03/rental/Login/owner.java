...
public class owner implements ActionListener{
    ...
    public owner() {
        frame = new JFrame("Owner Registration");
        ...
        JLabel ownerlLabel = new JLabel("OWNER REGISTRATION");
        ownerlLabel.setBounds(100, 70, 350, 30);
        frame.add(ownerlLabel);
        ownerlLabel.setFont(new Font("Times New Roman", Font.BOLD, 25));
        ownerlLabel.setForeground(Color.BLACK);
        ownerlLabel.setOpaque(false);
...
frame.setIconImage(icon.getImage());
    ...
    private void register() {
        ...
        UserOwner newUser = new UserOwner(name, email, password);

        try {
            FileWriter writer = new FileWriter("userOwnerdata.txt", true);
    ...
    private boolean ifEmailExists(String email) {
        try {
            File file = new File("userOwnerdata.txt");
            if (!file.exists()) {
                return false;
            }
